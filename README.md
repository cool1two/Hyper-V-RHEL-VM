...
The first step in using Hyper-V on your computer is to enable Virtualization in the BIOS.

- On the **Intel** platform, the option name is `Intel VT-x`
- On the **AMD** platform, the option name is `AMD-V`
...
/!\ By enabling Hyper-V, you may have difficulties with another **Hypervisor Software**, such as VirtualBox or VMWare.

You can restart your computer to apply the new configuration.

## Add user to Hyper-V Administrators group

Adding a user to the Hyper-V Administrators group is easy and useful.
You will be able to **create/manage/delete/start/stop** VMs without asking Administrators' permission.

First, you need to know your Windows **Username**.
To do that, you can run **PowerShell** as a user.
...
For the previous version of Windows :
...
Just follow the instructions given in this guide.
...
Please pay attention to the `Before you begin` section, point `1` if you want a no-cost Red Hat Developer subscription and the RHEL Binary DVD .iso file.

To have a free RedHat developer subscription [RedHat no-cost subscription](https://developers.redhat.com/blog/2021/02/10/how-to-activate-your-no-cost-red-hat-enterprise-linux-subscription#)
...
The last step is to set the `EnhancedSessionTransportType` to `HVSocket` for your VM. This can be done by running the following command in PowerShell as a user, replacing `<VM_NAME>` with the name of your VM.

```bash
Set-VM "<VM_NAME>" -EnhancedSessionTransportType HVSocket
```

Now, you can start your VM. After a few moments, you should have the following windows open.
...
You are done with the settings, and you can now connect to your VM.
Note that it is not possible to connect with a user already connected in another session.
...
Audio is not working by default for RHEL 8. To enable VM audio on the host computer, you need to install the xrdp module for PulseAudio.

### Prerequisites

Allow 2G of disk space for the mock root in `/var/lib/mock`.

It is assumed that you will build and install the package using an existing user account with sudo privileges.
...
The build is made in a sub-directory of `/var/lib/mock`. This can be fairly deep, so it is best located with a `find` command:
...
The last step in installing the Xrdp module is to place the module files in the correct location. In the same `pulseaudio-module-xrdp` directory from the previous step:

```bash
sudo make install
```

To load the XRDPM module, you need to add instructions in the PulseAudio configuration file. The following command line appends new instructions to the end of the configuration file.
